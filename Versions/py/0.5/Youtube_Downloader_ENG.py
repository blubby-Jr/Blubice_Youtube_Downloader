...
if not os.path.exists(FFMPEG_PATH):
    messagebox.showerror("Error", "FFmpeg not found in folder FFmpeg")
    sys.exit(1)
...
class YouTubeDownloaderPro(ctk.CTk):
    ...
    def __init__(self):
        ...
        ctk.CTkLabel(top, text="Link:").grid(row=0, column=0, padx=6)
        ...
        self.thumb_label = ctk.CTkLabel(mid, text="Miniature", width=260, height=160)
        self.thumb_label.grid(row=0, column=0, rowspan=4, padx=8)

        ctk.CTkLabel(mid, text="Name:").grid(row=0, column=1, sticky="nw")
        ...
        self.folder_btn = ctk.CTkButton(bot, text="📁 Folder", command=self.select_folder)
        ...
        self.cancel_btn = ctk.CTkButton(bot, text="❌ Cancel", fg_color="red", command=self.cancel)
    ...
    def start_check(self):
        ...
        if not url:
            messagebox.showwarning("Error", "Enter link")
            return

        self.check_btn.configure(state="disabled")
        self.status_label.configure(text="Getting metadata...")
    ...
    def start_download(self):
        if not self.info:
            messagebox.showwarning("Warning", "First click 🔍")
            return
    ...
    def progress_hook(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            if dt > 0:
                ...
                self.status_label.configure(text=f"Downloading: {speed:.2f} MB/s")
                self.last_time = now
