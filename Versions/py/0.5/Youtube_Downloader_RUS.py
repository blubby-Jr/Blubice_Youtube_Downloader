...
class YouTubeDownloaderPro(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.quality_menu = ctk.CTkOptionMenu(mid, variable=self.quality_var, values=["Лучшее"])
        ...
        self.folder_btn = ctk.CTkButton(bot, text="📁 Выбрать папку", command=self.select_folder)
        ...
        self.cancel_btn = ctk.CTkButton(bot, text="❌ Отменить", fg_color="red", command=self.cancel)
    ...
    def cancel(self):
        ...
        self.status_label.configure(text="Отмена загрузки...")
    ...
    def fetch_metadata(self, url):
        ...
        def ui():
            ...
            self.quality_menu.configure(values=["Лучшее"] + qualities + ["Только аудио"])
    ...
    def start_download(self):
        if not self.info:
            messagebox.showwarning("Предупреждение", "Сначала нажмите кнопку 🔍")
            return
    ...
    def progress_hook(self, d):
        if self.cancel_download:
            raise yt_dlp.utils.DownloadError("Загрузка отменена")
        ...
        if d['status'] == 'downloading':
            ...
            if dt > 0:
                ...
                self.status_label.configure(text=f"Загрузка: {speed:.2f} МБ/с")
                self.last_time = now
