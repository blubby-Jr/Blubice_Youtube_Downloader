...
class YouTubeDownloaderPro(ctk.CTk):
    def __init__(self):
        ...
        paste_btn = ctk.CTkButton(top_frame, text="📋 Insert link", command=self.paste_url)
        paste_btn.grid(row=1, column=1, padx=6, pady=4, sticky="w")

        self.check_btn = ctk.CTkButton(top_frame, text="🔍 Check", command=self.start_check)
        ...
        self.thumb_label = ctk.CTkLabel(mid_frame, text="Miniature\n(no)", width=260, height=160, anchor="center", justify="center")
        self.thumb_label.grid(row=0, column=0, rowspan=4, padx=8, pady=8)

        ctk.CTkLabel(mid_frame, text="Name:", anchor="w").grid(row=0, column=1, sticky="w")
        self.title_label = ctk.CTkLabel(mid_frame, text="", anchor="w", wraplength=480)
        self.title_label.grid(row=0, column=2, sticky="w", padx=4)

        ctk.CTkLabel(mid_frame, text="Duration:", anchor="w").grid(row=1, column=1, sticky="w")
        self.duration_label = ctk.CTkLabel(mid_frame, text="—", anchor="w")
        self.duration_label.grid(row=1, column=2, sticky="w", padx=4)

        ctk.CTkLabel(mid_frame, text="Available qualities:", anchor="w").grid(row=2, column=1, sticky="nw", pady=(6,0))
        ...
        self.folder_btn = ctk.CTkButton(bot_frame, text="📁 Select folder", command=self.select_folder)
        self.folder_btn.grid(row=0, column=0, padx=6, pady=8, sticky="w")
        self.folder_label = ctk.CTkLabel(bot_frame, text=f"Folder: {self.folder_path}", anchor="w")
        self.folder_label.grid(row=0, column=1, columnspan=3, sticky="w")

        self.format_label = ctk.CTkLabel(bot_frame, text="Download as:", anchor="w")
        self.format_label.grid(row=1, column=0, sticky="w", padx=6)
        self.save_type_var = ctk.StringVar(value="MP4 (video)")
        ...
        self.download_btn = ctk.CTkButton(bot_frame, text="⬇️ download", command=self.start_download, width=160)
    ...
    def start_check(self):
        ...
        if not url:
            messagebox.showwarning("Error", "Enter the link to the video!")
            return
        self.check_btn.configure(state="disabled")
        self.status_label.configure(text="Loading metadata...")
    ...
    def fetch_metadata(self, url):
        try:
            ...
            self.after(0, lambda: (messagebox.showerror("Error", f"Failed to retrieve metadata:\n{e}"), self.check_btn.configure(state="normal"), self.status_label.configure(text="")))
            return

        self.info = info
        title = info.get('title', 'No Name')
        ...
        def ui_update():
            ...
            self.status_label.configure(text="Done. Select the quality and click Download..")
            ...
            if thumbnail:
                try:
                    ...
                    self.thumb_label.configure(text="Miniature\n(error)")
            else:
                self.thumb_label.configure(text="Miniature\n(no)")
    ...
    def start_download(self):
        if not self.info:
            messagebox.showwarning("Error", "Check the link first (🔍 Check).")
            return
        url = self.url_entry.get().strip()
