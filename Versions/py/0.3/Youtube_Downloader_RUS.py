...
# ---------- Путь к ffmpeg рядом с exe/скриптом ----------
...
if not os.path.exists(FFMPEG_PATH):
    raise FileNotFoundError(f"FFmpeg не найден в папке {os.path.dirname(FFMPEG_PATH)}")
...
class YouTubeDownloaderPro(ctk.CTk):
    ...
    def __init__(self):
        ...
        ctk.CTkLabel(top, text="Ссылка на YouTube:").grid(row=0, column=0, padx=6)
        self.url_entry = ctk.CTkEntry(top, width=560)
        self.url_entry.grid(row=0, column=1, padx=6)

        paste = ctk.CTkButton(top, text="📋 Вставить", command=self.paste_url)
        paste.grid(row=1, column=1, sticky="w", padx=6, pady=4)

        self.check_btn = ctk.CTkButton(top, text="🔍 Проверить", command=self.start_check)
        ...
        ctk.CTkLabel(mid, text="Название:").grid(row=0, column=1, sticky="w")
        ...
        self.quality_menu = ctk.CTkOptionMenu(mid, variable=self.quality_var, values=["best"], width=200)
    ...
    def start_check(self):
        ...
        self.status_label.configure(text="Загрузка информации...")
    ...
    def fetch_metadata(self, url):
        ...
        qualities = sorted({str(f.get("height")) for f in formats if f.get("height")}, key=lambda x: int(x))
    ...
    def start_download(self):
        if not self.info:
            messagebox.showwarning("Ошибка", "Сначала проверьте видео")
            return
