...
if not os.path.exists(FFMPEG_PATH):
    raise FileNotFoundError(f"FFmpeg not found in folder {os.path.dirname(FFMPEG_PATH)}")
...
class YouTubeDownloaderPro(ctk.CTk):
    ...
    def __init__(self):
        ...
        paste = ctk.CTkButton(top, text="📋 insert", command=self.paste_url)
        paste.grid(row=1, column=1, sticky="w", padx=6, pady=4)

        self.check_btn = ctk.CTkButton(top, text="🔍 Get Metadata", command=self.start_check)
        ...
        ctk.CTkLabel(mid, text="Name:").grid(row=0, column=1, sticky="w")
    ...
    def start_download(self):
        if not self.info:
            messagebox.showwarning("Error", "Check the video first")
            return
